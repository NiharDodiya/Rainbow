...
class ViewStoryController extends GetxController {
  ...
  Future<void> commentData(String id) async {
    try {
      loader.value = true;
      storyCommentModel = (await StoryCommentApi.sendNewComment(id,writeSomethings.text.toString()) ?? StoryCommentModel());
  ...
  void onCommentButtonTap( {String? id}) {
    StoryCommentsController storyController = Get.put(StoryCommentsController());
